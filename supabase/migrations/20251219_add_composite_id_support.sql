-- Migration to add composite ID support for user collections
-- This ensures proper variant-level data isolation

-- Add index for faster lookups by product_id + variant_id combination
CREATE INDEX IF NOT EXISTS idx_user_collections_product_variant 
ON user_collections(tcgplayer_product_id, card_variant_id);

-- Add index for user + product + variant lookups
CREATE INDEX IF NOT EXISTS idx_user_collections_user_product_variant 
ON user_collections(user_id, tcgplayer_product_id, card_variant_id);

-- Update the collection search function to return composite IDs
CREATE OR REPLACE FUNCTION get_user_collection_cards_with_composite_ids(user_id_param uuid)
RETURNS TABLE (
  id uuid,
  composite_id text,
  user_id uuid,
...
AS $$
  SELECT 
    uc.id,
    -- Generate composite ID in format: productId_variantId
    CONCAT(uc.tcgplayer_product_id, '_', uc.card_variant_id) AS composite_id,
    uc.user_id,
    uc.portfolio_id,
    ...
    -- Price data for THIS SPECIFIC VARIANT
  ...
  FROM user_collections uc
     ...
     -- Get price for the SPECIFIC variant (critical for data isolation)
     LEFT JOIN LATERAL (
       ...
       WHERE ph.product_id = uc.tcgplayer_product_id
         AND ph.variant_id = uc.card_variant_id  -- MUST match the exact variant
       ORDER BY ph.recorded_at DESC
       LIMIT 1
     ) variant_price ON true
     -- Fallback to variant 1 (Normal) if specific variant price not found
     LEFT JOIN LATERAL (
       SELECT ph.market_price, ph.low_price, ph.mid_price, ph.high_price, ph.recorded_at
       FROM price_history ph
       WHERE ph.product_id = uc.tcgplayer_product_id
...
-- Add comment explaining the composite ID system
COMMENT ON FUNCTION get_user_collection_cards_with_composite_ids IS 
'Returns user collection with composite IDs in format productId_variantId for frontend compatibility. Ensures variant-level price and quantity isolation.';
